...
// an unused template that is never referenced by any ExecuteTemplate
// call or {{template}} action.
...
// resolved template.
func checkCalls(pkg *packages.Package, pending []pendingCall, resolved map[types.Object]*resolvedTemplate, inspectCall ExecuteTemplateNodeInspectorFunc, inspectTemplate TemplateNodeInspectorFunc, warn WarningFunc) error {
	...
	if warn != nil {
		for obj, rt := range resolved {
			...
			for _, name := range names {
				...
				warn(pos, fmt.Sprintf("template %q is defined but never referenced", name))
			}
		}
	}
